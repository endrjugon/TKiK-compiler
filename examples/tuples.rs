...
fn divmod_pair(a: Value, b: Value) -> Value {
    return Value::Tuple(vec![(a).div(&(b)), (a).rem(&(b))]);
    Value::None
}

fn swap(a: Value, b: Value) -> Value {
    return Value::Tuple(vec![b.clone(), a.clone()]);
    Value::None
}

fn main() {
    let mut point = Value::Tuple(vec![Value::Int(3), Value::Int(4)]);
    let __t0 = point.clone();
    let mut x = __t0.index(&Value::Int(0));
    let mut y = __t0.index(&Value::Int(1));
    let _ = py_print(&[Value::Str("x:".to_string()), x.clone(), Value::Str("y:".to_string()), y.clone()]);
    let mut a = Value::Int(1);
    let mut b = Value::Int(2);
    let __t1 = Value::Tuple(vec![b.clone(), a.clone()]);
    a = __t1.index(&Value::Int(0));
    b = __t1.index(&Value::Int(1));
    let _ = py_print(&[Value::Str("swapped:".to_string()), a.clone(), b.clone()]);
    let mut pair = Value::Tuple(vec![Value::Int(10), Value::Int(20)]);
    let _ = py_print(&[Value::Str("first:".to_string()), (pair).index(&(Value::Int(0))), Value::Str("second:".to_string()), (pair).index(&(Value::Int(1)))]);
    let mut singleton = Value::Tuple(vec![Value::Int(42)]);
    let _ = py_print(&[Value::Str("singleton:".to_string()), (singleton).index(&(Value::Int(0)))]);
    let mut pairs = Value::list(vec![Value::Tuple(vec![Value::Int(1), Value::Int(2)]), Value::Tuple(vec![Value::Int(3), Value::Int(4)]), Value::Tuple(vec![Value::Int(5), Value::Int(6)])]);
    for __item2 in (pairs).iter_values() {
        let mut left = __item2.index(&Value::Int(0));
        let mut right = __item2.index(&Value::Int(1));
        let _ = py_print(&[(left).add(&(right))]);
    }
}
