...
impl Counter {
    ...
    fn increment(&mut self) -> Value {
        self.value = (self.value).add(&(Value::Int(1)));
        Value::None
    }
...
fn squares(n: Value) -> Value {
    return { let mut __acc0: Vec<Value> = Vec::new(); for i in (py_range(&[n.clone()])).iter_values() { __acc0.push((i).mul(&(i))); } Value::list(__acc0) };
    Value::None
}

fn sum_list(xs: Value) -> Value {
    let mut total = Value::Int(0);
    for x in (xs).iter_values() {
        total = (total).add(&(x));
    }
    return total.clone();
    Value::None
...
fn main() {
    let mut nums = Value::list(vec![Value::Int(1), Value::Int(2), Value::Int(3), Value::Int(4), Value::Int(5)]);
    let mut evens = { let mut __acc1: Vec<Value> = Vec::new(); for x in (nums).iter_values() { if (((x).rem(&(Value::Int(2)))).eq_(&(Value::Int(0)))).is_truthy() { __acc1.push(x.clone()); } } Value::list(__acc1) };
    let _ = py_print(&[Value::Str("evens:".to_string()), evens.clone()]);
    let _ = py_print(&[Value::Str("squares:".to_string()), squares(Value::Int(5))]);
    let _ = py_print(&[Value::Str("sum:".to_string()), sum_list(nums.clone())]);
    let mut c = Counter::new(Value::Int(10));
    let _ = (c).increment();
    let _ = (c).increment();
    let _ = py_print(&[Value::Str("counter:".to_string()), (c).get()]);
    let mut scores = Value::dict(vec![(Value::Str("alice".to_string()), Value::Int(90)), (Value::Str("bob".to_string()), Value::Int(85))]);
    let _ = py_print(&[Value::Str("alice:".to_string()), (scores).index(&(Value::Str("alice".to_string())))]);
    let mut uniq = Value::set(vec![Value::Int(1), Value::Int(2), Value::Int(2), Value::Int(3)]);
    let _ = py_print(&[Value::Str("uniq size:".to_string()), py_len(&(uniq))]);
    let mut i = Value::Int(0);
    while ((i).lt(&(Value::Int(3)))).is_truthy() {
        if ((i).eq_(&(Value::Int(1)))).is_truthy() {
            let _ = py_print(&[Value::Str("one".to_string())]);
        } else {
            let _ = py_print(&[Value::Str("other".to_string())]);
        }
        i = (i).add(&(Value::Int(1)));
    }
}
