...
fn classify(n: Value) -> Value {
    if ((n).lt(&(Value::Int(0)))).is_truthy() {
        return Value::Str("negative".to_string());
    } else if ((n).eq_(&(Value::Int(0)))).is_truthy() {
        return Value::Str("zero".to_string());
    } else {
        return Value::Str("positive".to_string());
    }
    Value::None
}
...
fn factorial(n: Value) -> Value {
    ...
    let mut i = Value::Int(2);
    while ((i).le(&(n))).is_truthy() {
        result = (result).mul(&(i));
        i = (i).add(&(Value::Int(1)));
    }
    return result.clone();
...
fn main() {
    let mut x = Value::Int(7);
    let mut y = Value::Int(5);
    let _ = py_print(&[Value::Str("sum:".to_string()), add(x.clone(), y.clone())]);
    let _ = py_print(&[Value::Str("class:".to_string()), classify((x).sub(&(Value::Int(12))))]);
    let _ = py_print(&[Value::Str("factorial:".to_string()), factorial(x.clone())]);
}
